...
`ai-medical` 是一个基于 LangGraph 的多 Agent 医疗预问诊系统。它接收患者的症状描述，先执行确定性风险初筛，再通过多轮问答收集信息；需要医学知识辅助时，系统会只读查询 MySQL、Chroma 和 Neo4j，形成“症状→候选疾病及关联症状→确定性方向筛选→方向关联检查”的可解释闭环，最后生成结构化预问诊摘要与病历草稿。
...
- 预问诊、方向筛选和摘要共用同一套症状语义规则，统一处理“没有/否认/无”等否定表达及“发烧→发热、呼吸困难→气促”等别名，同时避免把“无力、无尿”等阳性症状误判为否定。
- 通过 MySQL 实体映射、Chroma 语义召回和 Neo4j 图谱查询生成可追溯的 `GraphEvidence`。
- 症状鉴别检索只扩展候选疾病和其他关联症状；确定性规则最多追加 2 轮图谱鉴别追问，患者已否认的症状不会重复询问。
- 只有同一方向获得至少 2 个已确认症状支持时才展示，最多 3 个方向；阴性症状作为冲突信息而非一票否决。
- 检查仅来自最终保留方向的图谱证据，每个方向最多 3 项，不表述为必须执行的医嘱。
- 摘要和病历草稿只整理患者已经提供的事实；图谱候选单独标为“需由医生进一步排查的方向”，不写成患者诊断。
- React 单页前端在首轮采集可选的年龄、性别和妊娠信息，并展示风险初筛、追问轮次、排查方向、可能评估的检查及各终态。
- 提供受配置控制的安全只读 trace，用于展示检索意图、标准化方式、图谱关系、稳定 ID 和来源记录。

## 结果展示示例

下图来自当前 React 页面的一次真实本地运行。演示进程启用了 `LLM_ENABLED=true`、`RAG_ENABLED=true` 和 `DEMO_TRACE_ENABLED=true`，并使用 `deepseek-chat` 完成结构化分诊与信息采集；MySQL、Chroma 和 Neo4j 均读取当前正式数据产物。

完成态展示三阶段进度、结构化摘要、明确否认和待补充信息。只有获得至少 2 项已确认症状共同支持的图谱方向才会显示；相关检查仅从最终保留方向继续检索，不表示诊断或检查医嘱。

![ai-medical 启用 LLM 与 RAG 的预问诊完成态](assets/result-demo.png)

展开“真实知识参考”后，可查看本轮真实检索意图、实体标准化方式与分数、Neo4j 关系、稳定实体 ID 和来源记录。下图中的内容来自当前会话 trace，不是前端填充的示例证据。

![ai-medical 三库 RAG 安全 trace](assets/result-rag-trace.png)

> 截图使用不含真实身份信息的虚构症状数据，仅用于说明系统交互和可追溯检索结果；所有排查方向仍需由医生结合面诊进一步判断。
...
多轮预问诊信息采集
  ...
  │       症状 → 候选疾病 → 其他关联症状
  │         ↓
  │       HAS_SYMPTOM 证据写入 differential_evidence
  │         ↓
  │       最多 2 轮未确认关键症状追问
  └─ （基础信息充分且无待确认关键症状）或达到总轮数上限
       ↓
确定性聚合阳性/阴性症状并计算方向支持度
  ├─ 无最终方向 → 跳过检查检索
  └─ 有最终方向 → 仅以至多 3 个最终方向执行 disease_to_check
                         ↓
                       RECOMMENDS_CHECK 证据写入 check_evidence
       ↓
结构化摘要、排查方向、可能评估的检查和病历草稿
  ↓
completed
```
...
`src/domain/symptom_semantics.py` 是预问诊、方向筛选和摘要共用的确定性语义边界。槽位继续保留患者原始措辞，便于摘要忠实展示；需要匹配、去重或检索时才调用共享函数完成以下处理：

- 解析阳性、阴性和空表达，并从“否认气促”“没有胸痛”“无气促”等内容中提取具体阴性症状；
- 保护“无力”“无尿”“无汗”“无痛性”等本身以“无”开头的阳性医学词；
- 把“发烧”统一为“发热”、“头疼”统一为“头痛”、“呼吸困难/喘不上气/呼吸急促”统一为“气促”；
- 按局部短句判断否定范围，例如“没有发热，但有呕吐”只否定发热。

别名表只处理明确维护的同义表达，未知症状在去除首尾空白后原样保留。该规则用于信息一致性处理，不负责判断疾病，也不会把图谱扩展症状写成患者已经确认的事实。
...
- 数据库或索引在请求期间不可用时，RAG 会记录可审计的降级错误并继续基础预问诊；新增方向和检查字段返回空数组，不会生成伪造证据。应用依赖整体无法初始化时仍返回明确的 `503`。
...
| `MAX_QUESTION_COUNT` | 总追问硬上限 | 默认 `8` |
| `MAX_DIFFERENTIAL_QUESTION_COUNT` | 图谱鉴别追问上限 | 默认 `2` |
...
- `differential_directions`：至多 3 个需由医生进一步排查的方向，包含支持症状、冲突阴性症状、支持数、冲突数、净支持分和固定非诊断提示；净支持分为支持数减冲突数，单一宽泛症状或证据不足时为空数组。
- `possible_evaluations`：只包含已保留方向关联的图谱检查，记录对应方向、证据来源和来源记录；RAG 关闭或图谱不可用时为空数组。

这两个字段只用于预问诊信息整理，不表示患者患有某疾病，也不构成检查医嘱。
...
前端只在 `sessionStorage` 保存当前 `session_id`，不持久化患者消息正文。首轮可填写年龄、性别和妊娠状态，这些资料只随第一条消息提交，用于风险分诊和特殊人群识别；后续页面会显示建议科室及已追问轮次。点击“新预问诊”会生成新的会话 ID 并清空页面内存状态。输入框支持 Enter 发送、Shift+Enter 换行，并在中文输入法组合期间避免误发送。

## 典型使用场景

### 普通多轮预问诊

1. 打开 `/app/`，按需填写年龄、性别和妊娠状态，再输入“发热、咳嗽两天，晚上更明显”。
2. 按页面问题继续回答，例如“最高 38.5℃，没有胸痛，也没有呼吸困难”。
3. 页面沿用同一个 `session_id`，先显示 `waiting_user`，信息充分后显示 `completed`。
4. 最终页面展示预问诊摘要、阳性和阴性信息、缺失信息、证据充分时的排查方向与可能评估检查，以及病历草稿。

### 真实知识辅助预问诊

1. 确认三库已经导入正式数据，设置 `RAG_ENABLED=true` 和 `DEMO_TRACE_ENABLED=true`。
2. 打开 `/app/`，输入“发热、咳嗽两天”，并按页面问题继续确认或否认关联症状。
3. 展开“真实知识参考”。
4. 页面应显示标准化路径、分数、Neo4j 关系、稳定 ID 和来源记录；若至少两个已确认症状共同支持同一方向，完成页还会展示该排查方向及其关联检查。

在线鉴别意图为 `symptom_to_differential`：输入症状经 MySQL/Chroma 标准化后，使用稳定症状 ID 在白名单查询中读取候选疾病和疾病的其他关联症状，不提前读取检查。信息采集结束后，`differential_selection` 使用已确认阳性、明确阴性和 `HAS_SYMPTOM` 证据确定性筛选至多 3 个方向；只有存在最终方向时，工作流才以这些疾病名触发 `disease_to_check`，并把 `RECOMMENDS_CHECK` 证据单独写入 `check_evidence`。现场结果必须来自当前 API trace；若依赖不可用、trace 关闭或证据没有稳定 ID，该链路不能视为成功。
...
- LLM 失败时使用已经定义的规则或模板回退；症状鉴别检索失败时记录 `differential_retrieval_degraded` 并以空方向继续基础摘要；检查检索失败时记录 `check_retrieval_degraded`，保留已筛选方向并以空检查列表完成。数据契约或应用依赖初始化失败则明确报错，任何路径都不伪造证据。
...
- 排查方向采用简单、可审计的图谱共现计数，不是疾病概率，也未经过临床有效性验证。
