...
`ai-medical` 是一个基于 LangGraph 的多 Agent 医疗预问诊系统。它接收患者的症状描述，先执行确定性风险初筛，再通过多轮问答收集信息；需要医学知识辅助时，系统会只读查询 MySQL、Chroma 和 Neo4j，形成“症状→候选方向→关联症状/检查”的可解释闭环，最后生成结构化预问诊摘要与病历草稿。
...
- 组合检索一次扩展候选疾病、其他关联症状和检查；确定性规则最多追加 2 轮图谱鉴别追问，患者已否认的症状不会重复询问。
...
下图来自当前 React 页面的一次本地规则问诊完成态，展示三阶段进度、结构化摘要、明确否认和待补充信息。

> 截图使用虚构症状数据，并在关闭 LLM/RAG 的本地模式下生成；它用于展示普通预问诊界面，不作为真实三库 RAG 验收证据。

![ai-medical 普通预问诊完成态截图](assets/result-demo.png)
...
在线组合意图为 `symptom_to_differential`：输入症状经 MySQL/Chroma 标准化后，使用稳定症状 ID 在一次白名单查询中读取候选疾病、疾病的其他症状和关联检查。现场结果必须来自当前 API trace；若依赖不可用、trace 关闭或证据没有稳定 ID，该链路不能视为成功。
...
- LLM 失败时使用已经定义的规则或模板回退；运行期知识检索失败时记录 `knowledge_retrieval_degraded` 后继续基础流程，数据契约或应用依赖初始化失败则明确报错，任何路径都不伪造证据。
